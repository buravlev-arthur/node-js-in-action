...
_DerbyJS_ - полностековый фреймворк для realtime веб-приложений. Поддерживает синхронизацию и серверную визуализацию представлений (_MongoDB_, _Redis_).
...
_Flatiron.js_ - веб-фреймворк MVC, обеспечивающий свою реализацию маршрутизации, мидлваров. Поддерживает журналирование, плагины, управление данными.
Все модули логически разделены так, что их можно по-отдельности использовать в собственных проектах.

Установка и запуск:

```bash
npm install -g flatiron
flatiron create project-folder
cd project-folder && npm install
npm start
```

Библиотека маршрутизации: `Director`. Может строить таблицу маршрутизации на стороне сервера, клиента и для параметров в командной строке (bash).

**Определение таблицы маршрутов через класс в "Director"**: ./flatiron-example/router.js

**Определение параметров запроса в "Director" через регулярные выражения**: ./flatiron-example/matching.js

**Обработка потоков данных в "Director"**: ./flatiron-example/stream.js

**REST API в стиле Express и через группировку Director**: ./flatiron-example/matching.js

_Flation_ предоставляет REST-обёртку _Resourceful_, которая поддерживает _CouchDB_, _MongoDB_, _Socket.IO_.

**Плюсы**: полностью независимые модули (_Wintson_ для ведения журнала, _Director_ для маршрутизации и т.д.). Маршрутизатор _Director_ изоморфлен (использутеся на клиенте и на сервере). Упрощенный потоковый API _Director_ (в отличии от _Express_) с генерация событий до/после запросов. Менеджер плагинов.

**Минусы**: Сложность настройки в больших проектах в сравнеии с тем же фреймворком _Sails_.

**Пример проекта**: ./flatiron-example

## LoopBack

_LoopBack_ - API-фреймворк для мобильных и настольных платформ, но включает ORM, MVC, WebSockets. Включает веб-интерфейс для управления REST API. 

Установка и запуск:

```bash
# cli-утилита для управления LoopBack проектами
npm i -g strongloop
# создание и первый проекта
slc loopback
cd project-name
node .
```

Определение нового маршрута средствами Express через экземляр LoopBack-класса Router: `./loopback-example/server/boot/routes.js`. Такой способ применяется
для частных случаев. В LoopBack маршруты генерируются при создании моделей.

Создание модели:
```bash
# после ответа на все вопросы генератора и задания свойств
# модуль и описание модели появятся в соотвествующих директориях: server/models или common/models
slc loopback:model model-name
```

Добавление свойства модели
```bash
slc loopback:property
```

**Пример проекта**: ./loopback-example

**Плюсы**: избавление от рутинного написания кода при генерации API - всё делается через командную строку. Свобода для выбора фреймворка/библиотеке на клиентской стороне.
Разделение API-маршрутов для клиентской и серверной сторон. [Интеграция API для iOS и Android](https://loopback.io/doc/en/lb3/Client-SDKs.html).

**Минусы**: Описание модели на базе JSON отличается от большинства API баз данных. Уровень HTTP _LoopBack_ базируется на _Express_ и органичен данной библиотекой.
Отсутствует система плагинов. Возможно использование Express-мидлвар, но это не столько удобно, как в _hapi_ и _Flatiron_.
